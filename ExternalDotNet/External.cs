...
namespace ExternalDotNet
{
    public class External
    {
        ...
        /// <summary>
        /// Reads an enum from memory
        /// </summary>
        /// <typeparam name="T">The enum type</typeparam>
        /// <param name="address">The address to read from</param>
        /// <returns>The enum</returns>
        /// <exception cref="MemoryReadException">Thrown if memory read fails</exception>
        /// <exception cref="ArgumentException">Thrown if the value can't be cast to the enum type</exception>
        public T ReadEnum<T>(IntPtr address)
        {
            if (Is64Bit)
            {
                Int64 val = ReadInt64(address);
                return (T) Enum.ToObject(typeof(T), val);
            }
            else
            {
                Int32 val = ReadInt32(address);
                return (T)Enum.ToObject(typeof(T), val);
            }
        }
        ...
        public void Write<T>(IntPtr address, T value) where T : struct
        {
            ...
            IntPtr structurePtr = Marshal.AllocHGlobal(typeSize);
            // Convert to pointer
            Marshal.StructureToPtr(value, structurePtr, false);
            // Allocate bytes
            byte[] bytes = new byte[typeSize];
            // Copy to byte array
            Marshal.Copy(structurePtr, bytes, 0, typeSize);
            // Free pointer
            Marshal.FreeHGlobal(structurePtr);
        ...
        public async Task<IntPtr> ExecuteAndWait(Int32 address, CallingConvention callingConvention, params IntPtr[] arguments)
        {
            ...
            assembler.FunctionCall((Int32)address, callingConvention, args);
            assembler.Return();
        ...
        public async Task<IntPtr> ExecuteAndWait(Int64 address, CallingConvention callingConvention, params IntPtr[] arguments)
        {
            ...
            assembler.FunctionCall((Int64)address, args);
            assembler.Return();
        ...
        public async Task<IntPtr> ExecuteAndWait(IntPtr address, CallingConvention callingConvention, params IntPtr[] arguments)
        {
            if (Is64Bit) // 64 bit
            {
                return await ExecuteAndWait((Int64) address, callingConvention, arguments);
            }
            else // 32 bit
            {
                return await ExecuteAndWait((Int32)address, callingConvention, arguments);
            }
        }
    }
}